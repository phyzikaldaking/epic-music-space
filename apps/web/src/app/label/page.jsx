...
export default async function LabelListPage() {
    ...
    return (<div className="mx-auto max-w-5xl px-4 py-12">
      <div className="mb-10 flex items-center justify-between">
        ...
        <a href="/label/new" className="rounded-xl bg-brand-500 px-5 py-2.5 text-sm font-semibold hover:bg-brand-600 transition">
          + Create Label
        </a>
      </div>

      {labels.length === 0 ? (<div className="py-24 text-center text-white/30">
          ...
          {labels.map((label) => {
                var _a;
                return (<a key={label.id} href={`/label/${label.id}`} className="glass rounded-2xl p-5 transition hover:border-brand-500/50">
              <div className="mb-3 flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-white/50">
                    by {(_a = label.owner.name) !== null && _a !== void 0 ? _a : "Unknown"}
                  </p>
                </div>
              </div>
              {label.bio && (<p className="mb-3 text-xs text-white/50 line-clamp-2">{label.bio}</p>)}
