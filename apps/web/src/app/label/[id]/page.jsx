...
export default async function LabelDetailPage({ params }) {
    ...
    if (!label)
        notFound();
    const isOwner = ((_a = session === null || session === void 0 ? void 0 : session.user) === null || _a === void 0 ? void 0 : _a.id) === label.ownerId;
    const myMembership = label.artists.find((la) => { var _a; return la.artistId === ((_a = session === null || session === void 0 ? void 0 : session.user) === null || _a === void 0 ? void 0 : _a.id); });
    return (<div className="mx-auto max-w-4xl px-4 py-12">
      ...
      <div className="mb-8 flex items-center gap-5">
        ...
        <img src={label.logoUrl} alt={label.name} className="h-full w-full object-cover"/>) : ("🏷️")}
        </div>
        <div>
          <h1 className="text-3xl font-extrabold">{label.name}</h1>
          <p className="text-sm text-white/50">
            Owned by{" "}
            <a href={((_b = label.owner.studio) === null || _b === void 0 ? void 0 : _b.username)
            ? `/studio/${label.owner.studio.username}`
            : "#"} className="text-brand-400 hover:underline">
              {(_c = label.owner.name) !== null && _c !== void 0 ? _c : "Unknown"}
            </a>
          </p>
        </div>
      </div>

      {label.bio && (<p className="mb-8 text-sm text-white/60">{label.bio}</p>)}
      ...
      <div className="mb-8 grid grid-cols-3 gap-4">
        {[
            ...
            { label: "Label rev share", value: `${String(label.revSharePct)}%` },
            {
                ...
                value: label.artists.reduce((s, la) => s + la.artist.songs.reduce((ss, x) => ss + x.soldLicenses, 0), 0),
            },
      ...
      <section className="mb-8">
        ...
        {label.artists.length === 0 ? (<div className="glass rounded-2xl p-6 text-center text-white/30 text-sm">
            No artists signed yet.
          </div>) : (<div className="overflow-x-auto rounded-2xl border border-white/10">
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                var _a, _b;
                return (<tr key={la.id} className="border-b border-white/5 hover:bg-white/5">
                    <td className="px-4 py-3">
                      <a href={((_a = la.artist.studio) === null || _a === void 0 ? void 0 : _a.username)
                        ? `/studio/${la.artist.studio.username}`
                        : "#"} className="font-medium text-brand-400 hover:underline">
                        {(_b = la.artist.name) !== null && _b !== void 0 ? _b : "Unknown"}
                      </a>
                    </td>
                    <td className="px-4 py-3">{String(la.revSharePct)}%</td>
                    <td className="px-4 py-3">
                      <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${la.status === "ACTIVE"
      ...
      {(myMembership === null || myMembership === void 0 ? void 0 : myMembership.status) === "PENDING" && (<LabelOfferActions labelId={id} revSharePct={String(myMembership.revSharePct)} labelName={label.name}/>)}

      {/* Owner actions */}
      {isOwner && (<section className="glass rounded-2xl p-5">
