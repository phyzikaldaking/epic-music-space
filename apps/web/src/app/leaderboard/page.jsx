...
export default async function LeaderboardPage({ searchParams, }) {
    ...
    return (<div className="mx-auto max-w-5xl px-4 py-12">
      ...
      <div className="mb-6 flex gap-3">
        <a href="/leaderboard?type=songs" className={`rounded-xl px-5 py-2 text-sm font-semibold transition ${type === "songs"
            ? "bg-brand-500 text-white"
            : "border border-white/20 text-white/60 hover:bg-white/10"}`}>
          🎵 Songs
        </a>
        <a href="/leaderboard?type=artists" className={`rounded-xl px-5 py-2 text-sm font-semibold transition ${type === "artists"
            ? "bg-brand-500 text-white"
            : "border border-white/20 text-white/60 hover:bg-white/10"}`}>
          🎤 Artists
        </a>
      ...
      <LeaderboardTable entries={type === "songs" ? songs : artistRows} type={type}/>
    </div>);
}
