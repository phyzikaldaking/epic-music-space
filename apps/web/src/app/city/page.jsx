...
export default async function CityPage() {
    ...
    function MiniSongCard({ song, }) {
        return (<a href={`/studio/${song.id}`} className="flex items-center gap-3 rounded-xl border border-white/8 bg-[#141414] p-3 transition card-hover-neon">
    ...
    function District({ district, songs, }) {
        const meta = DISTRICT_META[district];
        const visual = DISTRICT_VISUAL[district];
        return (<section className={`rounded-3xl border ${visual.border} ${visual.bg} overflow-hidden`}>
        {/* District header */}
        ...
        <div className="p-6">
          {songs.length === 0 ? (<div className="text-center py-8">
              <p className="text-sm text-white/25">No songs in this district yet.</p>
              {district !== "INDIE_BLOCKS" && (<a href="/pricing" className="mt-3 inline-block rounded-xl border border-brand-500/40 bg-brand-500/10 px-5 py-2 text-sm font-semibold text-brand-400 hover:bg-brand-500/20 transition">
                  Unlock this district →
                </a>)}
            </div>) : (<div className="grid gap-2 sm:grid-cols-2">
              {songs.map((s) => (<MiniSongCard key={s.id} song={s}/>))}
            </div>)}
        </div>
      </section>);
    }
    return (<div className="relative min-h-screen">
      ...
      <div className="relative mx-auto max-w-5xl px-4 py-12">
        ...
        ].map(({ district, icon, border, bg, textColor, lock }) => {
            const meta = DISTRICT_META[district];
            return (<div key={district} className={`rounded-2xl border ${border} ${bg} p-5 text-center`}>
                ...
                {lock && (<a href="/pricing" className={`mt-3 inline-block rounded-lg border ${border} px-3 py-1 text-xs font-semibold ${textColor} hover:opacity-80 transition`}>
                    Requires {lock}
                  </a>)}
              </div>);
        })}
        ...
        <div className="mt-12 rounded-3xl border border-brand-500/30 bg-brand-500/8 p-8 text-center">
          ...
          <a href="/pricing" className="inline-block rounded-xl bg-brand-500 px-8 py-3 text-sm font-bold text-white hover:bg-brand-600 transition glow-purple-sm">
            View pricing →
          </a>
        </div>
