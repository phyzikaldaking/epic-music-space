...
export default async function AdsPage({ searchParams, }) {
    ...
    return (<div className="relative min-h-screen">
      ...
      <div className="relative mx-auto max-w-6xl px-4 py-12">
        ...
        {sp.purchased && (<div className="mb-8 rounded-2xl border border-green-500/25 bg-green-500/10 p-4 text-green-400">
        ...
        ].map((s) => (<div key={s.label} className="rounded-2xl border border-white/8 bg-[#141414] p-5 card-hover-neon">
        ...
        <section className="mb-10">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {LOCATIONS.map((loc) => (<div key={loc.key} className={`rounded-2xl border ${loc.border} ${loc.bg} p-5 card-hover-neon`}>
        ...
        {/* ── Purchase form ─────────────────────────────── */}
        {session ? (<section className="mb-12 glass-card rounded-2xl p-6 border border-white/8">
            <h2 className="mb-5 text-xl font-bold">Purchase an Ad Slot</h2>
            <AdPurchaseForm />
          </section>) : (<section className="mb-12 rounded-2xl border border-brand-500/25 bg-brand-500/6 p-8 text-center">
            ...
            <a href="/auth/signin" className="inline-block rounded-xl bg-brand-500 px-6 py-2.5 text-sm font-bold text-white hover:bg-brand-600 transition glow-purple-sm">
              Sign in →
            </a>
          </section>)}
        ...
        <section>
          <div className="mb-5 flex items-center justify-between">
            ...
            {activeAds.length > 0 && (<span className="text-sm text-white/35">{activeAds.length} running</span>)}
          </div>
          {activeAds.length === 0 ? (<div className="rounded-2xl border border-white/8 bg-[#141414] p-8 text-center text-sm text-white/30">
              No active ad placements at the moment. Be the first!
            </div>) : (<div className="grid gap-4 sm:grid-cols-2">
              {activeAds.map((ad) => (<div key={ad.id} className="overflow-hidden rounded-2xl border border-white/8 bg-[#141414] card-hover-neon">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={ad.mediaUrl} alt={ad.title} className="h-36 w-full object-cover opacity-80"/>
                  <div className="p-4">
