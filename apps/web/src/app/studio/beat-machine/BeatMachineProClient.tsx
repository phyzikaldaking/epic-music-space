...
function Panel({ children, className = "" }: { children: React.ReactNode; className?: string }) { return <section className={cn("border border-white/10 bg-[#121519]/96 shadow-[inset_0_1px_0_rgba(255,255,255,.08),0_18px_50px_rgba(0,0,0,.34)]", className)}>{children}</section>; }
function Title({ children, right }: { children: React.ReactNode; right?: React.ReactNode }) { return <div className="flex h-10 items-center justify-between border-b border-white/10 bg-black/22 px-4 text-[11px] font-black uppercase tracking-[0.14em] text-white/82"><span>{children}</span>{right}</div>; }
function Button({ children, active, onClick }: { children: React.ReactNode; active?: boolean; onClick?: () => void }) { return <button onClick={onClick} className={cn("border px-3 py-1.5 text-[10px] font-black uppercase tracking-[0.12em] transition", active ? "border-cyan-300 bg-cyan-300 text-black shadow-[0_0_18px_rgba(32,247,255,.24)]" : "border-white/12 bg-white/[0.035] text-white/62 hover:border-cyan-300/50 hover:text-cyan-100")}>{children}</button>; }
...
function Sampler({ log, assign }: { log: (m: string) => void; assign: () => void }) { return <div className="grid min-h-0 flex-1 grid-rows-[auto_1fr] gap-3 overflow-hidden"><Panel className="p-4"><Wave /><div className="mt-4 flex flex-wrap gap-3"><Button onClick={() => log("Auto chop created six transient markers.")} active>Auto Chop</Button><Button onClick={() => log("Extracted six one-shots into the preview rack.")}>Extract One-Shots</Button><Button onClick={assign}>Assign To Pad</Button><Button onClick={() => log("Saved sample to My Sounds.")}>Save To My Sounds</Button></div></Panel><div className="grid gap-3 lg:grid-cols-3">{["One-Shots", "AI Suggestions", "Keyboard"].map((title) => <Panel key={title} className="overflow-auto p-4"><Title>{title}</Title><div className="mt-4 space-y-3">{[1,2,3,4].map((i) => <button key={i} onClick={() => log(`${title} item ${i} selected.`)} className="w-full rounded-xl border border-cyan-300/35 bg-black/30 p-3 text-left text-sm">{title} {i}<Wave compact /></button>)}</div></Panel>)}</div></div>; }
...
function Sounds({ assignSound, selected }: { assignSound: (s: string) => void; selected: string }) { const [q, setQ] = useState(""); const filtered = sounds.filter((s) => `${s.name} ${s.target} ${s.label}`.toLowerCase().includes(q.toLowerCase())); return <Panel className="min-h-0 flex-1 overflow-hidden p-4"><div className="mb-4 flex gap-3"><h2 className="text-3xl font-black uppercase text-cyan-200">My Sounds</h2><input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search sounds..." className="min-w-[260px] flex-1 rounded-full border border-cyan-300/50 bg-black/45 px-5 text-sm outline-none" /><span className="text-xs uppercase text-white/50">Selected {selected}</span></div><div className="grid max-h-[calc(100dvh-180px)] grid-cols-1 gap-4 overflow-auto sm:grid-cols-2 xl:grid-cols-3">{filtered.map((sound) => <button key={sound.name} onClick={() => assignSound(sound.name)} className="rounded-2xl border border-white/12 bg-[#191c1e] p-4 text-left"><Wave compact /><b className="mt-3 block text-[12px] uppercase">{sound.name}</b><p className="mt-2 text-[10px] uppercase text-white/45">Routes to {sound.target.toUpperCase()} pad.</p></button>)}</div></Panel>; }
