...
const sounds = ["HARD_KICK_01", "DARK_SNARE_07", "DEEP_808_12", "VOCAL_BAD_05", "LOFI_MELODY_02", "BRIGHT_CLAP_04", "MOODY_PAD_05", "TRAP_PERC_HIT_62", "BRIGHT_CHOP_03"];
...
export default function BeatMachineProClient({ initialView = "machine", studioMode = false }: { initialView?: View; studioMode?: boolean }) {
  ...
  function stop() { if (timer.current) window.clearTimeout(timer.current); timer.current = null; setPlaying(false); setStep(0); }
  function play() {
    if (playing) { stop(); return; }
    void context().resume();
    let cursor = 0;
    const tick = () => {
      const baseInterval = Math.max(40, (60 / bpmRef.current / 4) * 1000);
      const totalSteps = barsRef.current * 16;
      const laneIndex = cursor % 16;
      setStep(laneIndex);
      padsRef.current.forEach((pad) => { if (pad.steps[laneIndex]) trigger(pad, 0.88); });
      cursor = (cursor + 1) % totalSteps;
      const swungStep = cursor % 2 === 1;
      const swingOffset = swungStep ? baseInterval * (swingRef.current / 100) * 0.35 : -baseInterval * (swingRef.current / 100) * 0.2;
      timer.current = window.setTimeout(tick, Math.max(20, baseInterval + swingOffset));
    };
    timer.current = window.setTimeout(tick, Math.max(40, (60 / bpmRef.current / 4) * 1000));
    setPlaying(true);
    log(`Pattern playback started (${barsRef.current} bar${barsRef.current > 1 ? "s" : ""}, swing ${swingRef.current}%).`);
  }
