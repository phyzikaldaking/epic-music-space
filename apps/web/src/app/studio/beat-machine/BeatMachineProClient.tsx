    ...
    <div className="grid h-full min-h-0 grid-rows-[34px_1fr]">
      ...
      <main className="grid min-h-0 grid-cols-[360px_1fr] overflow-hidden">
        <section className="min-h-0 overflow-auto border-r border-black bg-[#111418] p-3">
          ...
          <div className="mt-4 border-t border-white/10 pt-4"><div className="mb-3 flex items-center justify-between"><b className="text-[11px] uppercase tracking-widest text-cyan-200">Live Sample Library</b><span className="font-mono text-[10px] text-white/35">{liveSamples.length} sounds</span></div><div className="max-h-52 space-y-1 overflow-auto pr-1">{liveSamples.slice(0, 80).map((name) => <button key={name} onClick={() => void loadSample(name)} disabled={sampleLoading} className={cn("flex w-full items-center justify-between border px-2 py-2 text-left text-[10px] font-bold uppercase", sampleName === name ? "border-cyan-300 bg-cyan-300/15 text-cyan-100" : "border-white/10 bg-black/25 text-white/60 hover:border-cyan-300/60 hover:text-white")}><span className="truncate">{name.replace(/\\.(wav|mp3|ogg|m4a)$/i, "")}</span><span className="ml-2 shrink-0 text-[9px] text-cyan-200">LOAD</span></button>)}</div>{sampleLoading && <p className="mt-2 text-[9px] font-bold uppercase text-cyan-200">Loading sample…</p>}{sampleError && <p className="mt-2 text-[9px] font-bold uppercase text-red-300">{sampleError}</p>}<p className="mt-2 text-[9px] leading-4 text-white/35">Samples load from Supabase Storage and replace the selected pad sound. Tap a pad, then load a sound.</p></div><div className="mt-4 border-t border-white/10 pt-4">
