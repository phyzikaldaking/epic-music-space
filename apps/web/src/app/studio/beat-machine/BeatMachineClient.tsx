...
export default function BeatMachineClient() {
  ...
  async function persistPattern(pattern: SavedPattern) {
    const res = await fetch("/api/studio/beat-patterns", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ projectId: PROJECT_ID, sessionId: SESSION_ID, ...pattern, arrangement: sections, pianoNotes }) });
    const data = await res.json().catch(() => null);
  ...
  async function savePattern() { const optimistic: SavedPattern = { id: `pattern-${Date.now()}`, name: `Pattern ${savedPatterns.length + 1}`, tracks: cloneTracks(tracks), bpm, swing, arrangement: sections, pianoNotes: cloneNotes(pianoNotes), createdAt: new Date().toISOString() }; setSyncing(true); try { const saved = await persistPattern(optimistic); setSavedPatterns((current) => [saved, ...current.filter((item) => item.id !== saved.id)].slice(0, 24)); setNotice(`${saved.name} saved to project.`); } catch (error) { setNotice(error instanceof Error ? error.message : "Backend save failed."); } finally { setSyncing(false); } }
      ...
      <section className="grid gap-3 xl:grid-cols-[320px_minmax(0,1fr)]">
        <aside className="space-y-3">
          <Panel title="Performance Pads" tone="green">
            <div className="grid grid-cols-2 gap-2">{PADS.map((pad) => <button key={pad.label} onClick={() => firePad(pad.kind, pad.label)} className={`h-16 rounded-xl border text-left transition ${activePad === pad.label ? "scale-95" : "hover:scale-[.98]"}`} style={{ background: pad.color, borderColor: pad.color, color: "#061014", boxShadow: activePad === pad.label ? `0 0 22px ${pad.color}` : undefined }}><span className="block px-3 text-base font-black uppercase">{pad.label}</span><span className="block px-3 text-[10px] font-black uppercase opacity-70">Key {pad.hotkey}</span></button>)}</div>
          </Panel>
