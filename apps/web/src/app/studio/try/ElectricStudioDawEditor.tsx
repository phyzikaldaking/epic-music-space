...
function formatRuler(seconds: number, mode: RulerMode) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (let x = 0; x < width; x += 1) {
        ...
        const barHeight = Math.max(1, Math.round(peak * (height - 2)));
        ctx.fillRect(x, Math.round(height / 2 - barHeight / 2), 1, barHeight);
      }
...
export default function ElectricStudioDawEditor() {
  ...
  const ticks = useMemo(() => {
    const step = rulerMode === "bars" ? 0.5 : 1;
    return Array.from({ length: Math.ceil(sessionEnd / step) + 1 }, (_, index) => Number((index * step).toFixed(3)));
  }, [rulerMode, sessionEnd]);

  useEffect(() => {
    const onKey = (event: KeyboardEvent) => {
      if ((event.target as HTMLElement | null)?.tagName === "INPUT") return;
      if (event.key === "+" || event.key === "=") setZoom((value) => Math.min(2400, value * 1.25));
      if (event.key === "-" || event.key === "_") setZoom((value) => Math.max(20, value / 1.25));
      if (event.key === "0") setZoom(160);
  ...
  function createTrack(kind: TrackKind) {
    if (kind === "master" && tracks.some((track) => track.kind === "master")) return setStatus("Master track already exists");
  ...
  function reflowShuffle(track: Track) {
    let cursor = 0;
    return { ...track, clips: [...track.clips].sort((a, b) => a.start - b.start).map((clip) => { const next = { ...clip, start: cursor }; cursor += visibleDuration(next); return next; }) };
  }

  function findClip(id: string | null) {
    if (!id) return null;
    return allClips.find(({ clip }) => clip.id === id) ?? null;
  }

  function setZoomSafe(next: number) {
    setZoom(Math.max(20, Math.min(2400, next)));
  }
  ...
  function handleTimelineWheel(event: React.WheelEvent) {
    ...
    setZoomSafe(zoom * factor);
    setStatus("Timeline zoom adjusted");
  }
  ...
  function handleTimelinePointerDown(event: React.PointerEvent) {
    ...
    if (timelinePointers.current.size === 2) lastPinchDistance.current = null;
  }

  function handleTimelinePointerMove(event: React.PointerEvent) {
    if (!timelinePointers.current.has(event.pointerId)) return;
    timelinePointers.current.set(event.pointerId, { x: event.clientX, y: event.clientY });
    const points = Array.from(timelinePointers.current.values());
    if (points.length < 2) return;
    const distance = Math.hypot(points[0].x - points[1].x, points[0].y - points[1].y);
    if (lastPinchDistance.current) setZoomSafe(zoom * (distance / lastPinchDistance.current));
    lastPinchDistance.current = distance;
  }

  function clearTimelinePointer(event: React.PointerEvent) {
  ...
  function startClipGesture(event: React.PointerEvent, trackId: string, clip: Clip, mode: DragMode) {
    ...
    if (clip.locked) return setStatus("Clip is locked");
    ...
    if (clip.groupId && mode === "move") {
      for (const { clip: item } of allClips) if (item.groupId === clip.groupId) originalGroupStarts[item.id] = item.start;
    }
  ...
  function moveDrag(event: React.PointerEvent) {
    ...
    setTracks((current) => current.map((track) => ({ ...track, clips: track.clips.map((clip) => {
      ...
      if (drag.mode === "trimStart") {
        ...
        return { ...clip, trimStart: trim, start: snapTime(drag.originalStart + (trim - drag.originalTrimStart), editMode) };
      }
  ...
  function endDrag(event: React.PointerEvent) {
    ...
    if (editMode === "spot" && drag.mode === "move") {
      ...
      if (Number.isFinite(parsed)) setTracks((current) => current.map((track) => ({ ...track, clips: track.clips.map((clip) => clip.id === drag.clipId ? { ...clip, start: Math.max(0, parsed) } : clip) })));
    }
    setDrag(null);
  ...
  function applyCrossfade() {
    ...
    if (!nextClip) return setStatus("No following clip to crossfade");
    const overlap = Math.max(0.08, Math.min(1.5, visibleDuration(clip) * 0.2, visibleDuration(nextClip) * 0.2));
    setTracks((current) => current.map((item) => item.id === track.id ? { ...item, clips: item.clips.map((candidate) => {
    ...
    <main className="grid h-full min-h-0 grid-rows-[auto_1fr_auto] overflow-hidden bg-[#101319] text-white">
      <header className="border-b border-black bg-[linear-gradient(180deg,#2b3038,#171b21)] px-3 py-3">
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2 text-[10px] font-black uppercase tracking-widest">
          ...
          {(["slip", "grid", "shuffle", "spot"] as EditMode[]).map((mode) => <button key={mode} onClick={() => { setEditMode(mode); setStatus(`${mode.toUpperCase()} mode`); }} className={cn("rounded-full px-3 py-2", editMode === mode ? "bg-yellow-300 text-black" : "bg-black/55 text-white/55 hover:text-white")}>{mode}</button>)}
          <span className="text-white/40">Ruler</span>
          {(["bars", "time", "samples", "timecode"] as RulerMode[]).map((mode) => <button key={mode} onClick={() => setRulerMode(mode)} className={cn("rounded-full px-3 py-2", rulerMode === mode ? "bg-cyan-300 text-black" : "bg-black/55 text-white/55 hover:text-white")}>{mode}</button>)}
          <button onClick={groupClipsOnTrack} disabled={!selectedClip} className="rounded-full bg-violet-300 px-3 py-2 text-black disabled:opacity-35">Group</button>
          ...
          <span className="ml-auto text-white/40">Zoom {Math.round(zoom)}px/sec</span>
          <button onClick={() => setZoomSafe(zoom / 1.5)} className="rounded-lg bg-black/55 px-3 py-2 text-white/65">-</button>
          <button onClick={() => setZoomSafe(zoom * 1.5)} className="rounded-lg bg-black/55 px-3 py-2 text-white/65">+</button>
          <button onClick={() => setZoomSafe(160)} className="rounded-lg bg-black/55 px-3 py-2 text-white/65">Reset</button>
        </div>
      </header>
      ...
      <section className="grid min-h-0 grid-cols-[270px_1fr_280px] overflow-hidden max-lg:grid-cols-[220px_1fr] max-md:grid-cols-1">
        ...
        <section className="grid min-h-0 grid-rows-[38px_auto_1fr_36px] overflow-hidden bg-[#10141a]" onWheel={handleTimelineWheel} onPointerDown={handleTimelinePointerDown} onPointerMove={handleTimelinePointerMove} onPointerUp={clearTimelinePointer} onPointerCancel={clearTimelinePointer} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); void importFiles(Array.from(event.dataTransfer.files)); }}>
          <div className="relative border-b border-black bg-[#30343b]" style={{ width: timelineWidth }}>
            {ticks.map((tick) => <button key={tick} onClick={() => setPlayhead(tick)} className="absolute bottom-0 top-0 border-r border-black/80 px-1 text-left font-mono text-[10px] text-white/50" style={{ left: tick * zoom, width: Math.max(40, zoom * (rulerMode === "bars" ? 0.5 : 1)) }}>{formatRuler(tick, rulerMode)}</button>)}
            <div className="absolute bottom-0 top-0 w-px bg-cyan-300" style={{ left: playhead * zoom }} />
          </div>
          <div className="flex items-center gap-3 border-b border-black bg-black/70 px-3 py-2 text-[10px] font-black uppercase tracking-widest text-white/45"><span>Wave vertical zoom {waveZoom.toFixed(2)}x</span><input type="range" min="0.35" max="4" step="0.05" value={waveZoom} onChange={(event) => setWaveZoom(Number(event.target.value))} className="max-w-xs flex-1 accent-yellow-300" /><span>Wheel/Pinch zoom timeline · +/- zoom · Q/W wave</span></div>

          <div className="overflow-auto">
            <div className="relative" style={{ width: timelineWidth }}>
              {tracks.map((track) => <div key={track.id} data-track-id={track.id} className={cn("relative h-[100px] border-b border-black bg-[#151a22] transition-colors", dropTargetTrackId === track.id && "bg-cyan-300/[0.08] ring-1 ring-inset ring-cyan-300/50")}>
                <div className="absolute inset-0">{ticks.map((tick) => <span key={tick} className={cn("absolute bottom-0 top-0 border-r", editMode === "grid" ? "border-cyan-300/25" : "border-black/70")} style={{ left: tick * zoom, width: Math.max(40, zoom * (rulerMode === "bars" ? 0.5 : 1)) }} />)}</div>
                {track.armed && <div className="absolute inset-0 bg-red-500/[0.045]" />}{track.muted && <div className="pointer-events-none absolute inset-0 z-10 bg-black/35" />}
        ...
        <aside className="min-h-0 overflow-auto border-l border-black bg-[#20242b] p-3 text-xs text-white/60 max-lg:hidden">
          <h2 className="text-[10px] font-black uppercase tracking-widest text-cyan-100">Regions / Inspector</h2>
