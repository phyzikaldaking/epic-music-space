...
export type StudioImportStatus = "queued" | "validating" | "decoding" | "uploading" | "peaks" | "loudness" | "complete" | "failed" | "relinked";
export type StudioImportMode = "protected" | "project-copy";
export type StudioMediaKind = "audio" | "video-audio" | "unknown";
...
export type StudioAudioFile = {
  ...
  format: string;
  normalizedFormat: "wav" | "mp3" | "aac" | "webm" | "flac" | "unknown";
  sizeBytes: number;
  duration: number;
  sampleRate: number;
  channels: number;
  ...
  storagePath: string;
  folderPath: string;
  waveformPeakId: string | null;
  uploaded: boolean;
  duplicateOfId: string | null;
  projectCopyImport: boolean;
  createdAt: string;
};
...
export type StudioWaveformPeak = {
  ...
  workerGenerated: boolean;
};

export type StudioImportJob = {
  id: string;
  fileName: string;
  status: StudioImportStatus;
  progress: number;
  retryCount: number;
  validation: string[];
  report: string[];
...
export type StudioRelinkRequest = {
  id: string;
  audioFileId: string;
  missingPath: string;
  status: "missing" | "selecting" | "relinked";
};

export type StudioMediaState = {
  importMode: StudioImportMode;
  backgroundWorkerEnabled: boolean;
  autoUploadEnabled: boolean;
  cloudPeakCacheEnabled: boolean;
  sampleRateConversionEnabled: boolean;
  normalizeFormatsEnabled: boolean;
  duplicateDetectionEnabled: boolean;
  loudnessScanEnabled: boolean;
  projectAudioRoot: string;
  importJobs: StudioImportJob[];
  audioFiles: StudioAudioFile[];
  waveformPeaks: StudioWaveformPeak[];
  relinkRequests: StudioRelinkRequest[];
};

export type StudioMarker = { id: string; name: string; position: number; color: string };
export type StudioMemoryLocation = { id: string; name: string; start: number; end: number };
export type StudioEditHistoryEntry = { id: string; label: string; createdAt: string };
...
export type StudioExportJob = { id: string; type: StudioExportType; status: StudioExportStatus; detail: string };
export type StudioCollaborator = { id: string; email: string; role: StudioRole; status: "active" | "invited" };
...
export type StudioState = {
  ...
  media: StudioMediaState;
};

export const initialStudioState: StudioState = {
  session: { id: null, title: "Untitled Studio Session", bpm: 120, sampleRate: 48000, dirty: false, savedAt: null, cloudSynced: false },
  ...
  clips: [],
  exports: [],
  collaborators: [{ id: "owner", email: "phyzikaldaking@gmail.com", role: "owner", status: "active" }],
  ...
  timeline: {
    ...
    commandSearch: "",
    markers: [{ id: "marker-intro", name: "Intro", position: 0, color: "#65d6ff" }, { id: "marker-hook", name: "Hook", position: 16, color: "#f9d66a" }],
    memoryLocations: [{ id: "memory-hook", name: "Hook Loop", start: 16, end: 32 }],
    undo: [],
    redo: [],
  },
  media: {
    importMode: "protected",
    backgroundWorkerEnabled: true,
    autoUploadEnabled: true,
    cloudPeakCacheEnabled: true,
    sampleRateConversionEnabled: true,
    normalizeFormatsEnabled: true,
    duplicateDetectionEnabled: true,
    loudnessScanEnabled: true,
    projectAudioRoot: "studio/projects/local/audio",
    importJobs: [],
    audioFiles: [],
    waveformPeaks: [],
    relinkRequests: [],
