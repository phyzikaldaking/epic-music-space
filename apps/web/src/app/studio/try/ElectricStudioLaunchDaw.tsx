...
export default function ElectricStudioLaunchDaw() {
  ...
  const syncLabel = syncState === "dirty" ? "Unsaved local changes" : syncState === "saving" ? "Saving to cloud..." : "Cloud synced";
  ...
  function queueExport(type: ExportType) {
    const job: ExportJob = {
      ...
      type,
      status: "queued",
      detail: type === "MP3 Demo"
        ? "Browser-rendered mix encoded to MP3 demo."
        : type === "WAV Master"
          ? "Browser OfflineAudioContext WAV master render queued."
          : type === "Stems"
            ? "Per-track rendered audio stems queued."
            : type === "Social Preview"
              ? "Audio plus visual preview render queued."
              : "Session archive bundle queued with metadata and audio.",
    };
  ...
  return (
    <div className="relative h-full overflow-hidden bg-[#05070a] text-white">
      ...
      <aside className="pointer-events-auto absolute bottom-20 left-3 top-16 z-[88] hidden w-[420px] overflow-hidden rounded-[1.35rem] border border-orange-300/20 bg-[#11151b]/96 shadow-[0_28px_90px_rgba(0,0,0,.76),inset_0_1px_0_rgba(255,255,255,.06)] backdrop-blur-xl 2xl:block">
        <header className="border-b border-black bg-[linear-gradient(180deg,rgba(251,146,60,.22),#171b21)] p-4">
          <p className="text-[10px] font-black uppercase tracking-[0.26em] text-orange-100/75">Main DAW / Launch Controls</p>
        ...
        <div className="max-h-[calc(100%-9rem)] overflow-auto p-4">
          <section className="rounded-2xl border border-white/10 bg-black/35 p-4">
            <h3 className="text-xs font-black uppercase tracking-widest text-cyan-100">Collaboration</h3>
              ...
              {collaborators.map((collab) => (
                <article key={collab.id} className="grid grid-cols-[1fr_120px] gap-2 border-b border-white/5 bg-[#11161d] p-3">
                  <div className="min-w-0">
                    <b className="block truncate text-[10px] uppercase tracking-widest text-white">{collab.email}</b>
                    <span className="text-[10px] uppercase tracking-widest text-white/40">{collab.status}</span>
                  </div>
          ...
          <section className="mt-4 rounded-2xl border border-white/10 bg-black/35 p-4">
            <div className="flex items-center justify-between gap-3">
              <div>
                <h3 className="text-xs font-black uppercase tracking-widest text-red-100">Editor Lock / Sync</h3>
                <p className="mt-1 text-xs text-white/50">{editorLocked ? `${lockOwner} is editing. Changes are view-only until released.` : "No active editor lock. You can edit safely."}</p>
              </div>
              <button onClick={() => setEditorLocked((value) => !value)} className={cn("rounded-xl px-3 py-2 text-[10px] font-black uppercase tracking-widest", editorLocked ? "bg-red-400 text-black" : "bg-[#222832] text-white/65")}>{editorLocked ? "Release" : "Sim Lock"}</button>
            </div>
            ...
            <div className="mt-3 grid grid-cols-3 gap-2 text-[10px] font-black uppercase tracking-widest">
              <button onClick={() => setSyncState("dirty")} className="rounded-xl bg-yellow-300 px-3 py-2 text-black">Dirty</button>
              <button onClick={saveSyncState} className="rounded-xl bg-cyan-300 px-3 py-2 text-black">Save</button>
              <button onClick={() => setSyncState("synced")} className="rounded-xl bg-green-300 px-3 py-2 text-black">Synced</button>
            </div>
          </section>

          <section className="mt-4 rounded-2xl border border-white/10 bg-black/35 p-4">
            <div className="flex items-center justify-between gap-3">
              <h3 className="text-xs font-black uppercase tracking-widest text-orange-100">Main Export Flow</h3>
              <button onClick={queueFullExportFlow} className="rounded-xl bg-orange-300 px-3 py-2 text-[10px] font-black uppercase tracking-widest text-black">Render All</button>
            </div>
            <div className="mt-3 grid grid-cols-2 gap-2 text-[10px] font-black uppercase tracking-widest">
              {exportTypes.map((type) => <button key={type} onClick={() => queueExport(type)} className="min-h-11 rounded-xl bg-[#222832] px-3 py-2 text-white/70 hover:bg-orange-300 hover:text-black">{type}</button>)}
            </div>
            ...
            <div className="mt-3 max-h-48 overflow-auto rounded-xl border border-white/10">
              {exportJobs.length === 0 ? <p className="p-3 text-xs text-white/45">No export jobs yet. Render MP3/WAV/stems/social/archive from here.</p> : exportJobs.map((job) => (
                <article key={job.id} className="border-b border-white/5 bg-[#11161d] p-3">
                  <div className="flex items-center justify-between gap-3">
                    <b className="text-[10px] uppercase tracking-widest text-white">{job.type}</b>
                    <span className={cn("rounded-full px-2 py-1 font-mono text-[9px]", job.status === "complete" ? "bg-green-300 text-black" : job.status === "rendering" ? "bg-yellow-300 text-black" : "bg-cyan-300/15 text-cyan-100")}>{job.status}</span>
                  </div>
                  <p className="mt-2 text-xs leading-5 text-white/45">{job.detail}</p>
                </article>
              ))}
            </div>
          </section>
          ...
          <section className="mt-4 rounded-2xl border border-white/10 bg-black/35 p-4">
            ...
            <p className="mt-2 text-xs text-white/50">Coverage: {coveredCount}/{testChecks.length}. These are the launch-path checks for import, record, edit, mix, save, upload, and export.</p>
            <div className="mt-3 grid gap-2">
              {testChecks.map((check) => (
                <button key={check.id} onClick={() => toggleCheck(check.id)} className={cn("flex items-center justify-between rounded-xl border px-3 py-3 text-left text-[10px] font-black uppercase tracking-widest", check.covered ? "border-green-300/35 bg-green-300/10 text-green-100" : "border-white/10 bg-[#11161d] text-white/45")}>
                  <span>{check.label}</span>
                  <span>{check.covered ? "Covered" : "Missing"}</span>
                </button>
              ))}
            </div>
          </section>
      ...
      <div className="absolute bottom-[4.7rem] left-3 right-3 z-[89] rounded-2xl border border-orange-300/20 bg-black/82 p-3 text-[10px] font-black uppercase tracking-widest text-white/60 shadow-[0_12px_40px_rgba(0,0,0,.45)] backdrop-blur 2xl:hidden">
        Launch controls are active: collaboration, roles, editor lock, sync states, MP3/WAV/stems/social/archive exports, server worker toggle, and E2E test coverage are available on a wider desktop view.
      </div>
    </div>
  );
}
