    ...
    <main className="min-h-[calc(100vh-65px)] overflow-hidden bg-[#05060b] text-white">
      <section className="border-b border-white/10 bg-black/80 px-3 py-2 shadow-2xl shadow-black/50 backdrop-blur">
        <div className="mx-auto flex max-w-[1500px] flex-wrap items-center gap-2">
          ...
          <nav className="ml-auto flex rounded-md border border-cyan-300/20 bg-cyan-300/5 p-1 text-[11px] font-black uppercase tracking-widest">
          ...
          {['AutoTune','EQ3','Compressor','De-Esser','Limiter'].map((plugin) => (
            <div key={plugin} className="mt-3 rounded-lg border border-white/10 bg-black/40 p-3">
              ...
              <div className="mt-3 grid grid-cols-3 gap-2">
                {[1,2,3].map((knob) => <div key={knob} className="mx-auto h-10 w-10 rounded-full border-4 border-cyan-300/60 bg-black shadow-inner" />)}
              </div>
            </div>
          ))}
        </aside>
            ...
            {tracks.map((track, index) => (
              <article key={track} className="rounded-lg border border-white/10 bg-black/50 p-2">
                ...
                <div className="grid grid-cols-2 gap-1 text-[9px] font-bold uppercase tracking-wider">
                  {['EQ','Comp','Tune','Send'].map((fx) => <button key={fx} className="rounded border border-cyan-300/20 bg-cyan-300/8 px-1 py-1 text-cyan-100">{fx}</button>)}
                </div>
                <div className="mt-3 flex h-36 items-end justify-center gap-2 rounded bg-white/[0.03] p-2">
                  <div className="h-full w-3 rounded bg-white/10"><div className="mt-auto h-2/3 rounded bg-emerald-400" /></div>
                ...
                <div className="mt-2 grid grid-cols-3 gap-1 text-[9px] font-black">
                  <button className="rounded bg-red-500/20 py-1 text-red-200">REC</button>
                  <button className="rounded bg-yellow-500/20 py-1 text-yellow-100">SOLO</button>
                  <button className="rounded bg-white/10 py-1 text-white/65">MUTE</button>
                </div>
              </article>
            ))}
          </div>
      ...
      <section id="beat" className="mx-auto max-w-[1500px] px-3 pb-6">
        <div className="rounded-xl border border-white/10 bg-[#090b14] p-3">
          ...
          <div className="grid gap-2">
            {lanes.map((lane, row) => (
              <div key={lane} className="grid grid-cols-[90px_repeat(16,minmax(28px,1fr))] gap-1">
                <div className="rounded bg-white/[0.05] px-2 py-2 text-xs font-black uppercase tracking-widest text-white/70">{lane}</div>
                {steps.map((step) => {
                  const active = (step + row) % (row === 2 ? 2 : 4) === 0;
                  return <button key={step} type="button" className={`h-10 rounded border ${active ? 'border-fuchsia-300 bg-fuchsia-400 text-black' : 'border-white/10 bg-black/50 text-white/30'}`}>{step}</button>;
                })}
              </div>
            ))}
          </div>
