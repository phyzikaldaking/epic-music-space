...
export default function StudioTryPage() {
  ...
  useEffect(() => {
    ...
    for (let lane = 0; lane < 6; lane += 1) {
      ...
      for (let x = 0; x < width; x += 8) {
        const amp = Math.sin((x / width) * Math.PI * 16 + lane) * (14 + lane * 2);
        const y = ((lane + 0.5) / 6) * height + amp;
  ...
  useEffect(() => {
    ...
    timerRef.current = window.setInterval(() => {
      void ensureAudio().then((ctx) => { if (!ctx || !masterRef.current) return; const when = ctx.currentTime + 0.012; lanes.forEach((lane) => { if (pattern[lane][step]) playSynthHit(ctx, lane, when, masterRef.current!); }); });
      setCurrentStep(step); step = (step + 1) % 16;
    }, interval);
    return () => { if (timerRef.current) window.clearInterval(timerRef.current); };
  }, [playing, bpm, currentStep, pattern]);

  const playLane = async (lane: Lane) => { const ctx = await ensureAudio(); if (!ctx || !masterRef.current) return; playSynthHit(ctx, lane, ctx.currentTime + 0.01, masterRef.current); };
