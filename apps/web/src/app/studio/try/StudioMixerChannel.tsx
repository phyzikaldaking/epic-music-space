...
import { EMS_VISUAL_SYSTEM, emsButton } from "./emsVisualSystem";
...
function StudioMixerChannel({ track, selected, playing = false, channelIndex = 1, onSelect, onUpdate, onAiMix, bindMeter }: Props) {
  ...
  return (
    <div
      ...
      className={`flex h-[780px] w-[126px] shrink-0 flex-col rounded-md border bg-gradient-to-b from-[#151b20] via-[#080c10] to-[#020304] p-1.5 shadow-[inset_0_1px_0_rgba(255,255,255,.08),inset_0_-18px_40px_rgba(0,0,0,.55),0_12px_30px_rgba(0,0,0,.38)] transition ${selected ? "ring-1 ring-cyan-200/60" : ""}`}
    ...
    >
      <div className="rounded-sm border border-white/10 bg-[#05080a] px-1.5 py-1 shadow-[inset_0_0_14px_rgba(0,0,0,.85)]">
        <div className="flex items-center justify-between">
          <span className="font-mono text-[8px] font-black text-white/35">CH {String(channelIndex).padStart(2, "0")}</span>
      ...
      <div className="mt-1.5 h-7 overflow-hidden rounded-sm border border-white/10 bg-black/90 shadow-[inset_0_0_18px_rgba(0,0,0,.85)]">
      ...
      <button
        ...
        className={`mt-1.5 rounded-sm border px-1 py-1 ${EMS_VISUAL_SYSTEM.text.tiny} ${EMS_VISUAL_SYSTEM.button.cyan}`}
      >
        AI MIX
      </button>

      <div className="mt-1.5 grid grid-cols-2 gap-1">
        {INSERTS.map((insert, index) => <button key={insert} onClick={(event) => event.stopPropagation()} className={`rounded-sm border px-1 py-1 ${EMS_VISUAL_SYSTEM.text.micro} ${index < 2 ? EMS_VISUAL_SYSTEM.button.cyan : EMS_VISUAL_SYSTEM.button.idle}`}>{insert}</button>)}
      </div>

      <div className="mt-1.5 grid grid-cols-3 gap-1">
        {SENDS.map((send, index) => <div key={send} className="rounded-sm border border-white/10 bg-black/45 p-1 shadow-[inset_0_0_10px_rgba(0,0,0,.65)]">
      ...
      <div className="mt-1.5 grid grid-cols-3 gap-1">
        <button onClick={(event) => { event.stopPropagation(); onUpdate({ muted: !track.muted }); }} className={`rounded-sm border py-1 ${EMS_VISUAL_SYSTEM.text.tiny} ${emsButton(track.muted, "magenta")}`}>M</button>
        <button onClick={(event) => { event.stopPropagation(); onUpdate({ solo: !track.solo }); }} className={`rounded-sm border py-1 ${EMS_VISUAL_SYSTEM.text.tiny} ${emsButton(track.solo, "gold")}`}>S</button>
        <button onClick={(event) => { event.stopPropagation(); onUpdate({ armed: !track.armed }); }} className={`rounded-sm border py-1 ${EMS_VISUAL_SYSTEM.text.tiny} ${emsButton(track.armed, "red")}`}>R</button>
      </div>
      ...
      <div className="mt-1.5 grid grid-cols-[16px_1fr] gap-2 rounded-sm border border-white/10 bg-[#040608] p-1.5 shadow-[inset_0_0_22px_rgba(0,0,0,.9)]">
        <div className="relative h-[370px] rounded-sm bg-black/90 ring-1 ring-white/10">
          ...
          <div ref={bindMeter?.(track.id)} className="absolute bottom-0 left-0 right-0 rounded-sm transition-[height] duration-75" style={{ height: `${track.muted ? 4 : track.meter}%`, background: `linear-gradient(to top, #22c55e 0%, #9cff2e 48%, #ffd166 72%, #ff2dcb 100%)`, boxShadow: active ? `0 0 14px ${track.color}` : undefined }} />
        </div>
        <div className="flex h-[370px] flex-col items-center justify-between">
          ...
          <span className="rounded-sm border border-white/10 bg-black/80 px-2 py-1 font-mono text-[8px] font-black text-cyan-100 shadow-[0_0_12px_rgba(34,211,238,.10)]">{track.volume}</span>
        </div>
      </div>
      ...
      <div className="mt-1.5 rounded-sm border border-white/10 bg-[#0b1014] px-1 py-1 text-center font-mono text-[7px] uppercase tracking-widest text-white/30">
        {track.muted ? "MUTED" : track.solo ? "SOLO" : track.armed ? "REC READY" : "READY"}
      </div>
    </div>
  );
}
