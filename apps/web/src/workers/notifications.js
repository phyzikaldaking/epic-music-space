...
if (!connection) {
    console.error("[notifications-worker] REDIS_URL is not set — worker cannot start");
    process.exit(1);
}
const worker = new Worker("ems:notifications", async (job) => {
    ...
    await prisma.notification.create({
        data: { userId, type, title, body, metadata: metadata !== null && metadata !== void 0 ? metadata : {} },
    });
...
worker.on("failed", (job, err) => {
    console.error(`[notifications-worker] Job failed: ${job === null || job === void 0 ? void 0 : job.id}`, err.message);
});
console.info("[notifications-worker] Started — listening for jobs on ems:notifications");
