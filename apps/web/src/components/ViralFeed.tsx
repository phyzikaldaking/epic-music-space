...
function buildLoopedFeed(clips: ViralClip[], pages: number) {
  return Array.from({ length: pages }).flatMap((_, pageIndex) =>
    clips.map((clip) => ({
      ...clip,
      id: `${clip.id}-page-${pageIndex}`,
    })),
  );
}

export default function ViralFeed() {
  const rankedClips = useMemo(() => rankViralClips(demoViralClips), []);
  ...
  return (
    <main className="relative h-screen overflow-hidden bg-black text-white">
      ...
      <header className="pointer-events-none fixed left-0 right-0 top-0 z-30 flex items-center justify-between px-5 py-5 md:px-8">
        <div>
        ...
        <div className="rounded-full border border-white/10 bg-black/45 px-4 py-2 text-xs font-black uppercase tracking-[0.16em] text-white/45 backdrop-blur-xl">
          Swipe Feed
        </div>
      </header>
        ...
        {clips.map((clip, index) => {
          ...
          return (
            <article
              ...
              className="relative flex h-screen snap-start items-center justify-center px-4 py-24 md:px-8"
            >
              <div className="grid w-full max-w-6xl gap-6 lg:grid-cols-[minmax(0,1fr)_360px] lg:items-center">
                  ...
                  <div className="aspect-[9/16] max-h-[74vh] w-full bg-black md:aspect-video md:max-h-[72vh]">
                    {clip.clipUrl ? (
                      ...
                      <div className="relative grid h-full place-items-center overflow-hidden bg-[radial-gradient(circle_at_30%_20%,rgba(253,224,71,0.20),transparent_30%),radial-gradient(circle_at_75%_55%,rgba(34,211,238,0.18),transparent_35%),linear-gradient(135deg,#101018,#030305)]">
                        ...
                        <div className={`relative rounded-full border border-gold-200/25 bg-gold-200/10 px-5 py-3 text-xs font-black uppercase tracking-[0.24em] text-gold-100 shadow-2xl shadow-gold-500/20 ${isActive ? "animate-pulse" : ""}`}>
                          Auto Clip Preview
                        </div>
                      </div>
                    )}
                ...
                <aside className="rounded-[2rem] border border-white/10 bg-black/42 p-5 shadow-2xl shadow-black/45 backdrop-blur-2xl">
                  <div className="flex items-center justify-between gap-3 border-b border-white/10 pb-4">
                    <div>
                      <p className="text-[10px] font-black uppercase tracking-[0.22em] text-white/35">Now Playing</p>
                      <p className="mt-1 text-xl font-black tracking-[-0.045em] text-white">{clip.artist ?? "EMS Artist"}</p>
                    </div>
                  ...
                  <div className="mt-5 grid gap-2">
                    <button className="rounded-2xl bg-white px-4 py-3 text-sm font-black uppercase tracking-[0.14em] text-black transition hover:bg-gold-200">Like Moment</button>
                    <button className="rounded-2xl border border-white/10 bg-white/[0.06] px-4 py-3 text-sm font-black uppercase tracking-[0.14em] text-white transition hover:bg-white/10">Share Clip</button>
                    <a href={clip.songId ? `/track/${clip.songId}` : "/marketplace"} className="rounded-2xl border border-gold-200/20 bg-gold-200/10 px-4 py-3 text-center text-sm font-black uppercase tracking-[0.14em] text-gold-100 transition hover:bg-gold-200/20">View Track</a>
                  </div>
