...
export default function VersusCard({ matchId, songA, songB, votesA: initialVotesA, votesB: initialVotesB, endsAt, userVotedSongId, }) {
    const [votesA, setVotesA] = useState(initialVotesA);
    const [votesB, setVotesB] = useState(initialVotesB);
    const [voted, setVoted] = useState(userVotedSongId !== null && userVotedSongId !== void 0 ? userVotedSongId : null);
    ...
    function SongSide({ song, votes, pct, side, }) {
        const isVoted = voted === song.id;
        const isLoser = voted && !isVoted;
        return (<button onClick={() => vote(song.id)} disabled={!!voted || isExpired || loading} className={`flex flex-1 flex-col items-center gap-3 rounded-2xl border p-5 transition
        ...
        <div className="relative h-32 w-32 overflow-hidden rounded-xl bg-gradient-to-br from-brand-900 to-accent-600 flex-shrink-0">
          {song.coverUrl ? (
            ...
            <img src={song.coverUrl} alt={song.title} className="h-full w-full object-cover"/>) : (<span className="flex h-full w-full items-center justify-center text-4xl">🎵</span>)}
          {side === "A" && (<span className="absolute top-2 left-2 rounded-md bg-brand-500 px-1.5 py-0.5 text-xs font-bold">A</span>)}
          {side === "B" && (<span className="absolute top-2 right-2 rounded-md bg-accent-500 px-1.5 py-0.5 text-xs font-bold">B</span>)}
        </div>
        ...
        {voted && (<div className="w-full">
            <div className="mb-1 flex justify-between text-xs text-white/60">
              <span>{votes} votes</span>
              <span className="font-bold text-white">{pct}%</span>
            </div>
            <div className="h-1.5 w-full rounded-full bg-white/10">
              <div className={`h-full rounded-full transition-all ${side === "A" ? "bg-brand-500" : "bg-accent-500"}`} style={{ width: `${pct}%` }}/>
            </div>
          </div>)}
      </button>);
    }
    return (<div className="rounded-3xl border border-white/10 bg-white/[0.03] p-6">
      {/* Header */}
      ...
      {!voted && !isExpired && (<p className="mt-4 text-center text-xs text-white/30">
