      ...
      <div className="overflow-x-auto overscroll-x-contain">
        <div style={{ width: controlWidth + timelineWidth, minWidth: "100%" }}>
          ...
          <div className="grid min-h-20 border-b border-white/10" style={{ gridTemplateColumns: `${controlWidth}px ${timelineWidth}px` }}>
            ...
            <button type="button" aria-label={`Place pattern ${selectedBank} on arrangement`} onClick={(event) => { const rect = event.currentTarget.getBoundingClientRect(); placeClip(Math.floor((event.clientX - rect.left) / barWidth) + 1); }} className="relative min-h-20 cursor-crosshair text-left" style={{ backgroundImage: `repeating-linear-gradient(to right, rgba(255,255,255,.045) 0, rgba(255,255,255,.045) 1px, transparent 1px, transparent ${barWidth}px)` }}>
              {clips.map((clip) => <span key={clip.id} onClick={(event) => { event.stopPropagation(); setSelectedClipId(clip.id); setSelectedBank(clip.bank); }} className={`absolute bottom-2 top-2 overflow-hidden rounded border px-2 py-2 text-[10px] font-black uppercase tracking-wider ${selectedClipId === clip.id ? "ring-2 ring-white" : ""}`} style={{ left: (clip.startBar - 1) * barWidth, width: clip.lengthBars * barWidth, borderColor: BANK_COLORS[clip.bank], color: BANK_COLORS[clip.bank], background: `linear-gradient(135deg, ${BANK_COLORS[clip.bank]}35, #050709 68%)` }}>Pattern {clip.bank}<small className="mt-1 block text-[8px] text-white/45">Bars {clip.startBar}–{clip.startBar + clip.lengthBars - 1}</small></span>)}
            </button>
          </div>
