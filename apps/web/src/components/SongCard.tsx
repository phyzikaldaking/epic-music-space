...
export default function SongCard({
  ...
  function startAnalyser() {
    ...
    const tick = () => {
      ...
      const nextSpectrum = visualizerBars.map((fallback, index) => {
        const value = data[index % data.length] ?? fallback;
        return Math.max(12, Math.min(100, (value / 255) * 100));
      });
  ...
  return (
    <article className={`group relative transform-gpu transition duration-500 [transform-style:preserve-3d] hover:z-20 hover:scale-[1.035] hover:rotate-y-0 ${wallTilt}`} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
      <div className="absolute -inset-3 rounded-[2rem] bg-gradient-to-br from-brand-500/0 via-accent-400/0 to-gold-300/0 opacity-0 blur-2xl transition duration-500 group-hover:from-brand-500/28 group-hover:via-accent-400/18 group-hover:to-gold-300/18 group-hover:opacity-100" style={{ opacity: playing ? Math.max(0.35, energy / 100) : undefined }} />
        ...
        <div className="relative aspect-[16/11] overflow-hidden rounded-[1rem] border border-white/10 bg-black shadow-inner shadow-black/90">
          {coverUrl ? (
            ...
            <div className="flex h-full w-full items-center justify-center bg-[radial-gradient(circle_at_20%_15%,rgba(108,92,231,.45),transparent_32%),radial-gradient(circle_at_80%_35%,rgba(0,245,255,.22),transparent_28%),linear-gradient(135deg,#171126,#061014)]">
              <svg aria-hidden="true" className="h-16 w-16 text-white/42" fill="currentColor" viewBox="0 0 24 24"><path d="M12 3v10.55A4 4 0 1 0 14 17V7h6V3h-8Z" /></svg>
            </div>
          )}
          ...
          <div className="absolute left-3 top-3 flex flex-wrap gap-1.5">{isTrending && <span className="badge-trending shadow-lg shadow-gold-500/20">Trending</span>}{isBoosted && <span className="badge-boosted shadow-lg shadow-brand-500/20">Boosted</span>}</div>
          <div className="absolute right-3 top-3 rounded-full border border-white/10 bg-black/60 px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.18em] text-white/55 backdrop-blur">{playing ? `Bass ${energy}%` : "EMS Screen"}</div>
          {aiScore !== undefined && aiScore > 0 && <div className="absolute bottom-3 left-3 flex items-center gap-1 rounded-full border border-white/10 bg-black/70 px-2.5 py-1 backdrop-blur"><span className="text-[10px] text-white/50">EMS</span><span className="text-xs font-bold text-brand-300">{aiScore.toFixed(1)}</span></div>}
          ...
          <div className="absolute inset-x-3 bottom-16 flex h-12 items-end gap-1 rounded-xl border border-white/8 bg-black/35 px-2 py-2 opacity-80 backdrop-blur-sm transition group-hover:opacity-100">
            {spectrum.map((height, index) => <span key={`${id}-bar-${index}`} className={`flex-1 rounded-t bg-gradient-to-t from-brand-500 to-accent-300 transition-all duration-75 ${playing ? "opacity-100" : "opacity-45"}`} style={{ height: `${height}%` }} />)}
          </div>
